    ...
    return (
        <div className="col-md-offset-1 col-md-6">
            ...
            <Thumbnail src={user.logo}>
                <h3>{user.name}</h3>
                <ul>
                    <li>{i18next.t('auth.user.email')}: {user.email}</li>
                    <li>{i18next.t('auth.user.provider')}: {user.provider_id}@{user.provider_type}</li>
                    <li>{i18next.t('auth.user.last_sign_in')}: {user.last_sign_in}</li>
                    <li>{i18next.t('auth.user.sign_in_count')}: {user.sign_in_count}</li>
                    <li>{i18next.t('auth.user.confirmed_at')}: {user.confirmed_at}</li>
                </ul>
    ...
    return <div className="col-md-offset-1 col-md-10">
      ...
      <div className="row">
        <div className="col-ms-3">
          <Button bsStyle="danger" onClick={this.handleClearCache}>{i18next.t('auth.clear_cache')}</Button>
        </div>
      </div>
    </div>
  }
...
const DashboardW = React.createClass({
  ...
  render () {
    ...
    if (isSignIn(user)) {
      var tabs = [(
        <Tab key="profile" eventKey={"profile"} title={i18next.t('auth.profile')}>
            <Profile/>
        </Tab>
          )]
      if (isAdmin(user)) {
        tabs.push(
            <Tab key="site.info" eventKey={"site.info"} title={i18next.t('auth.site_info')}>
                <div className="col-md-offset-1 col-md-10">
                    <br/>
                    <SiteInfoFm/>
                </div>
        ...
        tabs.push(
            <Tab key="site.notices" eventKey={"site.notices"} title={i18next.t('auth.notices')}>
                <Notices/>
            </Tab>
        )
        tabs.push(
            <Tab key="site.status" eventKey={"site.status"} title={i18next.t('auth.site_status')}>
                <StatusPane/>
            </Tab>
        )
      }
      tabs.push(
          <Tab key="logs" eventKey={"logs"} title={i18next.t('auth.logs')}>
              <Logs/>
          </Tab>
      )
      return (
